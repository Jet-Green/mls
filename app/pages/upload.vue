...
async function handleUpload() {
  if (!croppedImages.value.length) {
    toast.error('Выберите фото для загрузки')
    return
  }

  const files = croppedImages.value.map(item => new File([item.file], 'image.webp', { type: 'image/webp' }))
...
<template>
  <v-container class="py-8" style="max-width: 800px;">
    ...
    <v-card class="pa-6">
      ...
      <div v-if="croppedImages.length" class="mt-6">
        <v-row>
          <v-col v-for="(img, i) in croppedImages" :key="i" cols="6" sm="4" md="3">
